...
const mockNoFavPokemons = [];

describe('Testes do componente FavoritePokemons.js', () => {
  test('Testa se é exibida mensagem "No favorite pokemon found"'
  + ' se não houverem pokémons favoritados', () => {
    renderWithRouter(<FavoritePokemons pokemons={ mockNoFavPokemons } />);
  ...
  test('Testa se é exibido todos os cards de pokémons favoritados', () => {
    ...
    const firstPokemon = screen.getByRole('img', { name: 'Pikachu sprite' });
    const secondPokemon = screen.getByRole('img', { name: 'Charmander sprite' });
